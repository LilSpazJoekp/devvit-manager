...
CHANGES_FILE = Path("CHANGES.rst")
CONSTANTS_FILE = Path("devvit_mgr/const.py")
CHANGELOG_HEADER = (
    "Change Log\n==========\n\n"
...
def add_unreleased_to_changelog():
    ...
    CHANGES_FILE.write_text(f"{new_header}{content[len(CHANGELOG_HEADER):]}")
    return True
...
def increment_development_version():
    version = re.search('__version__ = "([^"]+)"', CONSTANTS_FILE.read_text()).group(1)
...
def main():
    ...
    if sys.argv[1] == "Unreleased":
        return not add_unreleased_to_changelog() and increment_development_version()
...
def valid_version(version):
    ...
    if parsed_version.local or parsed_version.is_postrelease or parsed_version.epoch:
        sys.stderr.write(
            "epoch, local, and postrelease version parts are not supported"
        )
        return False
